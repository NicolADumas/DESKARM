...
float global_a_est1;
float global_a_est2;
float global_q0_actual;
float global_q1_actual;


uint32_t global_counter1, global_counter2;
...
//    if((now - previous_trigger1) > DEBOUNCE_DELAY ){
       if (GPIO_Pin==LIMIT_SWITCH_1_Pin && homing_triggered){
			// if(!triggered1){
				  limit_switch1 = 1;
				  if(is_home1){
...
//	 if((now - previous_trigger2) > DEBOUNCE_DELAY ){
		if(GPIO_Pin==LIMIT_SWITCH_2_Pin && homing_triggered){
//			if(!triggered2){
				limit_switch2 = 1;
				if(is_home2){
...
//    global_displacement1 = displacement1;
//    global_displacement2 = displacement2;
}
...
void PID_controller_position(man_t *manip, pid_controller_t *pid1,pid_controller_t *pid2, float *u){
    ...
    */
	...
	global_q0_actual=measure1;
	global_q1_actual=measure2;
