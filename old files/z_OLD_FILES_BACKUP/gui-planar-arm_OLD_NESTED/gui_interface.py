...
@eel.expose
def py_get_emoji_list():
    return emoji_loader.get_available_emojis()

@eel.expose
def py_generate_emoji(emoji_char, size=0.05):
    print(f"Generating Emoji Trajectory: {emoji_char}")
    return emoji_loader.generate_emoji_trajectory(emoji_char, size)

@eel.expose
def py_save_custom_stamp(category, icon, path_data):
    print(f"Saving Custom Stamp: {icon} in {category}")
    return emoji_loader.save_custom_stamp(category, icon, path_data)
