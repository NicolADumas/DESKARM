# Catalogo Completo Codici Sorgente e Documentazione Tecnica

Questo documento aggrega le implementazioni software più significative del progetto e la documentazione tecnica dettagliata dei moduli principali.
...
### A.4 Approfondimento: Documentazione Libreria Traiettorie (`trajpy.py`)

Questo modulo (`trajpy.py`) fornisce gli strumenti fondamentali per la generazione di traiettorie e la cinematica del manipolatore.

**1. Generazione Polinomiale** [Interpolazione Temporale]
*   **`time_row(t, deg, der)`**: Calcola una riga della matrice di Vandermonde per la risoluzione di sistemi lineari di coefficienti polinomiali.
*   **`spline3(q, dq)`**: Calcola i coefficienti per una Spline Cubica (3° ordine) basata su posizione e velocità ai nodi.
    *   Profilo: $q(t) = a_0 + a_1t + a_2t^2 + a_3t^3$
*   **`spline5(q, dq, ddq)`**: Calcola i coefficienti per una Spline Quintica (5° ordine), permettendo il controllo anche dell'accelerazione.
*   **`compose_spline3(q, ddqm, dts)`**: Unisce più spline cubiche in una traiettoria continua.

**2. Profili di Velocità** [Leggi di Moto]
*   **`trapezoidal(q, ddqm, tf)`**: Genera un profilo trapezoidale (Acc. Costante -> Vel. Costante -> Dec. Costante). Se `tf` (tempo finale) non è fornito, calcola il tempo minimo (Bang-Bang).
*   **`cycloidal(q, ddqm, tf)`**: Genera una traiettoria cicloidale con accelerazione sinusoidale. Ideale per ridurre il jerk (scatti) e ottenere movimenti molto dolci.
    *   Formula base: $q(t) \approx t - \sin(t)$

**3. Cinematica** [Spazio Giunti <-> Spazio Cartesiano]
*   **`ik(x, y, z, ...)`**: Cinematica Inversa (vedi A.1).
*   **`dk(q, sizes)`**: Cinematica Diretta. Calcola $(x, y)$ dati gli angoli $(q_1, q_2)$.

**4. Utilities**
*   **`slice_trj(patch, **kargs)`**: Funzione core per il "slicing" (campionamento). Suddivide segmenti geometrici (linee, archi) in punti discreti temporizzati, applicando la cinematica inversa. Supporta ora parametro `angle` per archi precisi.
...
### C.3 Approfondimento: Documentazione Generatore Caratteri (`char_gen.py`)

Il modulo `char_gen.py` agisce come "Tipografo" del robot, convertendo testo in percorsi geometrici tracciabili.

#### Struttura Dati
*   **`FONT_DEFS`**: Dizionario contenente le definizioni vettoriali (A-Z, 0-9).
    *   **`line`**: Segmenti di linea.
    *   **`ellipse`**: Archi di ellisse parametrizzati.
    *   Le coordinate sono normalizzate (1x1) e scalate a runtime.

#### Funzioni Principali
*   **`text_to_traj(text, start_pos, ...)`**: Funzione master.
    1.  Itera sulla stringa.
    2.  Recupera primitive da `FONT_DEFS`.
    3.  Campiona curve (ellissi) in polilinee.
    4.  Applica scala e traslazione (World Coordinates).
    5.  Inserisce comandi `Pen Up` tra i tratti disgiunti.
*   **`sample_ellipse(center, radii, arc, steps)`**: Approssima archi di ellisse con punti densi per lettere curve (O, C, D).
*   **`get_char_strokes(char)`**: Recupera la definizione raw del carattere.
...
**File:** `main.py`
**Descrizione:** Inizializzazione server Eel e thread seriale.
...
**Descrizione:** Thread daemon per monitoraggio seriale asincrono.
...
**Descrizione:** Algoritmo adattivo per scelta tra Outline (Canny) e Skeleton (Morfologico) in base alla densità.
...
**Descrizione:** Verifica automatica del ridimensionamento immagini (tolleranza < 1cm).
